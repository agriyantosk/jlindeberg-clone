...
export const handleSubmitCategory = (category) => {
  return async (dispatch, getState) => {
    try {
      const response = await axios.post(`${baseUrl}/category/add`, category, {
        headers: {
          access_token: localStorage.access_token,
        },
      });
...
export const handleSubmitUser = (form) => {
  return async (dispatch, getState) => {
    try {
      const response = await axios.post(`${baseUrl}/author/register`, form);
      Swal.fire({
        position: "center",
        icon: "success",
...
export const handleLoginUser = (login) => {
  return async (dispatch, getState) => {
    try {
      const response = await axios.post(`${baseUrl}/author/login`, login);
